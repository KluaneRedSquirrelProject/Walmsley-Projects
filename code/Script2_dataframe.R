...
census[!is.na(def),topDef:=max(def),by=c("squirrel_id","year")]
census[!is.na(def),priority:=ifelse(def==topDef,"Y","N"),by=c("squirrel_id","year")]
# for census entries, identify priority territory location
census[!is.na(sq_fate),topFate:=min(sq_fate),by=c("squirrel_id","year")] # assuming 1 is priority if there happens to be a double entry
census[!is.na(sq_fate),priority:=ifelse(sq_fate==sq_fate,"Y","N"),by=c("squirrel_id","year")]
...
squirrelLocs <- census[census[ , .I[sample(.N,1)] , by = c('squirrel_id','year')]$V1]
...
distances[sex=="M",.N,by=is.na(maleARS)] # Two males with unknown paternity (not genotyped)
...
# for one individual (11220 in 2013), there are multiple equidistant nearest close kin (here, two daughters seemingly sharing a midden)
...
for (j in 1:nrow(noKin)) {
  ...
  neighbours <- subset(neighbours, neighbours$squirrel_id != noKin[j,squirrel_id,]) 
...
colnames(nK) <- c('focal','target','grid','distance','year','sex','age','mast','survival','all_litters_fit','maleARS','relationshipFine')
...
#write.csv(fullNH,"./output/nonKinDataset_socialNeighbourhood.csv")
...
for (j in 1:nrow(noKin)) {
  #this selects neighbours in the same grid, year, month and within 130 m of your focal individual.
  if (exists("neighbours")) neighbours <- NULL
  #neighbours <- NULL # clear
  neighbours <- subset(noKin, noKin$grid==noKin$grid[j] & noKin$year==noKin$year[j] & (sqrt((30*noKin$locX[j]-30*noKin$locX)^2 + (30*noKin$locY[j]-30*noKin$locY)^2) <= 1000))
  neighbours <- subset(neighbours, neighbours$squirrel_id != noKin[j,squirrel_id,])
  if (nrow(neighbours)>=1) {
    index <- sample(1:nrow(neighbours),1) # sample a neighbour at random
...
fullNH <- rbindlist(list(nearest,nK),fill=TRUE)
fullNH$relationshipFine <- factor(fullNH$relationshipFine,levels=c('unrelated','daughter','mother', 'sibling', 'son','father'))
levels(fullNH$relationshipFine) <- c("Non-kin", "Daughter", "Mother", "Sibling", "Son", "Father")

#write.csv(fullNH,"./output/nonKinDataset_allDistances.csv")
